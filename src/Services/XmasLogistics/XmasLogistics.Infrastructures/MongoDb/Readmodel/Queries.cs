...
using XmasLogistics.ReadModel.Abstracts;

namespace XmasLogistics.Infrastructures.MongoDb.Readmodel;
	...
	where T : EntityBase
{
	...
	public async Task<T> GetByIdAsync(string id, CancellationToken cancellationToken)
	{
		...
		return (await collection.CountDocumentsAsync(filter, cancellationToken: cancellationToken) > 0 ? (await collection.FindAsync(filter)).First() : null)!;
	}
